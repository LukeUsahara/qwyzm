...
## ADR-012: 対戦の権威は WebRTC ホストではなく WebSocket サーバーにする

ADR-005 を置き換える。Phase 11 で `apps/signaling` を権威ゲームサーバーにする。

理由:

- 早押し判定は改竄価値が高く、「クライアント値を無条件に信用しない」ならサーバーで確定するのが最短
- `game-core` は純関数 FSM なのでサーバー実行に流用でき、一人練習と規則が一致する
- サーバー時刻が単一基準になり、未接続だった同期時計も ping/pong で足りる
- WebRTC はシグナリングと P2P の二重実装、ホスト離脱時の権威移譲、TURN 運用が v1 を膨らませる

描画はサーバーが公開状態だけ送り、文字送りはクライアントの `getView(now)` で補間する。
